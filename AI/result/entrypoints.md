...
Functions whose access is *intentionally conditional* (e.g., signature-gated, or authentication is skipped on a specific path).
...
|----------|------|-----------------|
| `controlCollateral(address targetCollateral, address onBehalfOfAccount, uint256 value, bytes data)` | `src/EthereumVaultConnector.sol:L700` | The *sole enabled controller* for `onBehalfOfAccount` (`onlyController`) |
| `forgiveAccountStatusCheck(address account)` | `src/EthereumVaultConnector.sol:L884` | The *sole enabled controller* for `account` (`onlyController`) |
